...
export function sabziMandiBill(shoppingList, priceList) {
  ...
  if (
    Array.isArray(shoppingList) && shoppingList.length > 0 && typeof priceList == "object" && Object.values(priceList).length > 0
    ){
      let filteredShopList = shoppingList.filter(ele =>{ return typeof ele == "object" && Object.values(ele).length > 0 })
      if (filteredShopList.length <= 0) {
        return  { items: [], totalBill: 0 }      
      }
      let shopped = []
      let totalBill = 0
      for (const i of filteredShopList) {

        if (priceList[String(i.name)] && priceList[String(i.name)] <= 80) {
          let total =  Number(priceList[String(i.name)]) * Number(i.qty)
          shopped.push({
            name: i.name, 
            qty: i.qty, 
            cost:total
