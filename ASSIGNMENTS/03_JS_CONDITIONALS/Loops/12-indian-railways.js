...
export function railwayReservation(passengers, trains) {
  if (
    ...
    passengers.forEach((ele) => {
      let preferred = ele.preferred;
      let fallback = ele.fallback;
      let t_number = ele.trainNumber;
      let status = "";
      let t_class = "";

      let trainExist = trains.filter((ele) => ele.trainNumber == t_number);

      if (trainExist.length == 0) {
        final.push({ name:ele.name, trainNumber:t_number, class: null, status: "train_not_found" })
        return;
      }

      trains.forEach((train) => {
        if (train.trainNumber == t_number) {
      ...
      final.push({ name:ele.name, trainNumber:t_number, class: t_class, status: status })
    });
