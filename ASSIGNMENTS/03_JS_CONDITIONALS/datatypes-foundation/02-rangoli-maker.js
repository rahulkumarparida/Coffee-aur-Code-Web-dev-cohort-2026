...
if (typeof colorString !== "string") {
 ...
 let colorarr = colorString.split(oldSep).join(newSep)
...
export function replaceRangoliColor(design, oldColor, newColor) {
  ...
  let designArr = design.replaceAll(oldColor , newColor)
...
export function makeRangoliBorder(char, length) {
  ...
  let repeatpat = char.repeat(2*length)

  return repeatpat.slice(0,length)
