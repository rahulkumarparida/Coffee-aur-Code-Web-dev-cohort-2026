...
export function maskAadhaar(aadhaarNumber) {
 if (aadhaarNumber !== null && typeof aadhaarNumber == "string"  && aadhaarNumber.length === 12 ) {
  let lastdigit = aadhaarNumber.slice(-4)?aadhaarNumber.slice(-4): NaN
    if (isNaN(lastdigit)) {
      return "INVALID"
    }
    return "XXXX-".repeat(2)+lastdigit
