...
export function formatChaiMenu(items) {
  if (items !== null && Array.isArray(items) ) {
    let eachItem = items.filter(ele =>{
      return typeof ele.name == "string" && ele.name.length > 0 && typeof ele.price == "number" && ele.price > 0
    }).map(ele =>{
        return `${ele.name.toUpperCase()} - Rs.${ele.price}`
    })
