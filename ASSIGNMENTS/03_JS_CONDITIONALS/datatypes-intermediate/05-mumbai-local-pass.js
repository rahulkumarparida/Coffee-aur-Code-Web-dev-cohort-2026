 ...
 *   // => "INVALID PASS"
 */
let ex = { name: "rahul sharma", from: "dadar", to: "andheri", classType: "first" }

export function generateLocalPass(passenger) {
    function generatePassId(from , to , classType){
      return classType.charAt(0).toUpperCase()+from.slice(0,3).toUpperCase()+to.slice(0,3).toUpperCase()
    }
    function capitalize(word) {
    return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
  }
    if (
      passenger !== null && !Array.isArray(passenger) && typeof passenger == "object" &&
      typeof passenger.name == "string" && passenger.name.length > 0  &&
      typeof passenger.from == "string" && passenger.from.length > 0 &&
      typeof passenger.to == "string" && passenger.to.length >= 3 &&
      typeof passenger.classType == "string" && passenger.classType.length >= 3 &&
      ["first" , "second"].includes(passenger.classType.trim().toLowerCase())
    ) {
      let passid = generatePassId(passenger.from , passenger.to , passenger.classType)
      return `MUMBAI LOCAL PASS\n---\nName: ${passenger.name.toUpperCase()}\nFrom: ${capitalize(passenger.from)}\nTo: ${capitalize(passenger.to)}\nClass: ${passenger.classType.toUpperCase()}\nPass ID: ${passid}`
    }
    return "INVALID PASS"
